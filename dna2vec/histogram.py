...
class Histogram:
    ...
    def print_stat(self):
        for kmer_len in sorted(self.kmer_len_counter.keys()):
            self.logger.info(f'Percent of {kmer_len:2d}-mers: '
                             f'{100.0 * self.kmer_len_counter[kmer_len] / self.nb_kmers:3.1f}'
                             f'% ({self.kmer_len_counter[kmer_len]})')

        total_bps = sum([l * c for l, c in self.kmer_len_counter.items()])
        self.logger.info(f'Number of base-pairs: {total_bps}')
